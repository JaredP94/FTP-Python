...
The FTP server runs from the user's local host and allows users to connect through a locally hosted FTP client, as well as through clients on the same network. The server is also hosted on \textit{Google Cloud Services} and is thus accessible from any network using the IP address \textit{35.195.1.55}. The server facilitates the storage of files and a user management system and is capable of running on any Unix based system.
